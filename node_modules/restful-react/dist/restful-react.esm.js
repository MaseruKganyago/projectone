...
import { createElement, createContext, Component, Fragment, useEffect, useRef, useContext, useCallback, useState } from 'react';
...
var _fetchData = function _fetchData(props, state, setState, context, abortController) {
  try {
    var _temp5 = function _temp5(requestOptions) {
      function _temp3(contextRequestOptions) {
        var request = new Request(resolvePath(base, path, _extends({}, context.queryParams, {}, queryParams)), merge({}, contextRequestOptions, requestOptions, {
          signal: signal
        }));
        return _catch(function () {
...
function resolvePath(base, path, queryParams) {
  var appendedBase = base.endsWith("/") ? base : base + "/";
  var trimmedPath = path.startsWith("/") ? path.slice(1) : path;
  return url.resolve(appendedBase, Object.keys(queryParams).length ? trimmedPath + "?" + qs__default.stringify(queryParams) : trimmedPath);
}
...
function useGet() {
  ...
  return _extends({}, state, {
    absolutePath: resolvePath(props.base || context.base, props.path, _extends({}, context.queryParams, {}, props.queryParams)),
    ...
    refetch: function refetch(options) {
      ...
      return fetchData(_extends({}, props, {}, options), state, setState, context, abortController);
    }
  });
}
...
function useMutate() {
  ...
  var abortController = useRef(new AbortController()); // Cancel the fetch on unmount
    ...
    try {
      var _temp7 = function _temp7(propsRequestOptions) {
        function _temp5(contextRequestOptions) {
          ...
          var request = new Request(resolvePath(base, isDelete ? path + "/" + body : path, _extends({}, context.queryParams, {}, queryParams)), merge({}, contextRequestOptions, options, propsRequestOptions, mutateRequestOptions, {
            signal: signal
          }));
          var response;
